...
class tiempos extends Base_Controller{
	...
	// Crea tabla con listado de elementos capturados 
		...
		if($total_rows){
			foreach ($list_content as $value){
				...
				$ciclo = $this->db_model->db_get_tiempo_ciclo($value['id_nutricion_tiempo']);
				$eliminar 	= '<span style="color:red;" id="ico-eliminar_1" class="ico_eliminar fa fa-times" onclick="confirm_delete('.$value['id_nutricion_tiempo'].')" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['eliminar'] = (!$ciclo[0]['id_tiempo'])?$eliminar:'<span style="color:gray;" id="ico-eliminar_1" class="ico_eliminar fa fa-times" title="'.$this->lang_item("lbl_eliminar").'"></span>';
	...
	public function eliminar_registro(){
		$id_tiempo = $this->ajax_post('id_tiempo');
		$ciclo = $this->db_model->db_get_tiempo_ciclo($id_tiempo);
		if(!$ciclo[0]['id_tiempo']){
			$sqlData = array(
