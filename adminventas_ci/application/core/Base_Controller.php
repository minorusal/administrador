...
class Base_Controller extends CI_Controller {
    ...
    */
	public function build_panel_navigate($items, $uri, $sub = false, $bool = false) {
	    $panel    = "";
	    $style_ul = "";
	    if($sub){ if($bool){ $style_ul = "style='display: block;'";}else{$style_ul = "style='display: none;'"; } $panel .= "<ul class='nav child_menu' $style_ul>";}
	    foreach ($items as $item => $subitems) {
	        ...
	        if(array_key_exists('content', $subitems)){
	        	...
	        	$routes    = base_url();
	        	$icon      = $subitems['icon'];
	        	$sub_nivel = "<span class='fa fa-chevron-down'></span>";
	        }else{
	        	$routes    = "href='".base_url().$subitems['routes']."'";
	        	$icon      = $subitems['icon'] ;
	        }
	        $lang_item = $this->lang_item(str_replace(' ','_', $item));
	        $panel    .= "<li class = '$active' ><a $routes ><i  class='$icon'></i>".text_format_tpl($lang_item)." $sub_nivel </a>";
	        $panel    .= $content;
	       	$panel    .= "</li>";
	    }
	    if($sub){$panel .= "</ul>";}
