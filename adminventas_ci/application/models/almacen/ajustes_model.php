...
class ajustes_model extends Base_Model{
	...
	public function db_get_data($data=array()){	
		...
		$query = "SELECT 
					...
					IF(h.presentacion IS NOT NULL, CONVERT(CONCAT(IF(i.embalaje IS NOT NULL,CONCAT(i.embalaje,' CON '),''), IFNULL(g.presentacion_x_embalaje,''), ' ', IFNULL(h.presentacion,''), ' DE ', IFNULL(g.um_x_presentacion,''), ' ', IFNULL(f.clave_corta,'')) USING utf8),null) as presentacion_detalle
					...
					LEFT JOIN $tbl[compras_articulos_precios] g on a.id_compras_articulo_precios=g.id_compras_articulo_precios
					LEFT JOIN $tbl[compras_presentaciones] h on g.id_presentacion=h.id_compras_presentacion
	...
	public function get_data_stock($data=array()){	
		...
		$id_compras_articulo_precios  = ($data['id_compras_articulo_precios']!=0)?"AND c.id_compras_articulo_precios=$data[id_compras_articulo_precios]":'';
		...
		$query="SELECT 
				...
				WHERE a.activo=1 $id_articulo  $id_compras_articulo_precios $id_almacen $id_pasillo $id_gaveta
	...
	public function get_data_stock_logs($data=array()){	
		...
		$query="SELECT 
				...
				WHERE a.activo=1 AND a.id_accion=4 $id_almacen_ajuste $id_articulo $id_compras_articulo_precios $id_almacen $id_pasillo $id_gaveta
