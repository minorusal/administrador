...
class entidades extends Base_Controller
{
	...
	// Crea tabla con listado de elementos capturados 
		...
		if($total_rows){
			foreach ($list_content as $value){
				...
				$entidad    = $this->db_model->db_get_entidad($value['id_administracion_entidad']);
				...
				$btn_acciones['eliminar'] = (empty($entidad))?$eliminar:'<span style="color:gray;" id="ico-eliminar_'.$value['id_administracion_entidad'].'" class="ico_eliminar fa fa-times" title="'.$this->lang_item("lbl_eliminar").'"></span>';
	...
	public function eliminar_registro(){
		$id_entidad = $this->ajax_post('id_entidad');
		$entidad = $this->db_model->db_get_entidad($id_entidad);
		if(!$entidad[0]['id']){
			$sqlData = array(
