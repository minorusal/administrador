...
class presentaciones extends Base_Controller { 
	...
	public function listado_presentaciones($offset = 0){
		...
		$uri_view    = $this->uri_modulo.$this->uri_submodulo.'/'.$this->uri_seccion.'/listado_presentaciones';
		$limit       = 5;
		...
		if($total_rows>0){
			foreach ($lts_content as $value) {
				$atrr = array(
								'href' => '#',
							  	'onclick' => 'detalle_presentacion('.$value['id_cat_presentaciones'].')'
						);
	...
	public function detalle_presentacion(){
        ...
        $data_tab_3['presentaciones']        = $detalle_presentacion[0]['presentaciones'];
	...
	public function agregar_presentacion(){
		...
		$btn_save       = form_button(array('class'=>"btn btn-primary",'name' => 'save_articulo','onclick'=>'insert_presentacion()' , 'content' => $this->lang_item("btn_guardar") ));
	...
	public function insert_presentacion(){
		...
		if($incomplete>0){
			...
			$data_insert = array('presentaciones' => $presentacion,
	...
	public function update_presentacion(){
		...
		if($incomplete>0){
			...
			$data_update      = array('presentaciones' => $presentacion,
									  'clave_corta'    => $clave_corta, 
									  'descripcion'    => $descripcion);
